  ...
  return (
    <ContextMenu>
      ...
      <ContextMenuContent className="w-56">
        {latest.isDraft ? (
          <ContextMenuItem onSelect={() => void editDraftFromList(latest.id)}>
            <PenSquare />
            {t(locale, 'mail.editDraft')}
          </ContextMenuItem>
        ) : (
          <>
            <ContextMenuItem onSelect={() => void replyFromList(latest.id, false)}>
              <Reply />
              {t(locale, 'mail.reply')}
            </ContextMenuItem>
            <ContextMenuItem onSelect={() => void replyFromList(latest.id, true)}>
              <ReplyAll />
              {t(locale, 'mail.replyAll')}
            </ContextMenuItem>
            <ContextMenuItem onSelect={() => void forwardFromList(latest.id)}>
              <Forward />
