...
| `spam_settings` / `spam_sender` | `user_id` → `lyra_user.id` | Anti-spam preferences and learned sender lists (`0016_anti_spam`); `message.spam_verdict` records the engine's judgment — the post-sync pass judges once, but user actions (`spam` / `not_spam` / moves) and sender-list edits revise it (blocked list edits re-file the sender's inbox mail; allowed edits rescue mail Lyra itself filed as spam) |
