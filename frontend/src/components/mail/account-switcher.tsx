...
export function AccountSwitcher({ isCollapsed }: { isCollapsed: boolean }) {
  ...
  return (
    <Select value={selectedAccountId} onValueChange={setSelectedAccount}>
      <SelectTrigger
        className={cn(
          'flex w-full items-center gap-2 border-0 bg-transparent shadow-none hover:bg-accent/50 [&>span]:line-clamp-1 [&>span]:flex [&>span]:w-full [&>span]:items-center [&>span]:gap-1 [&>span]:truncate [&_svg]:h-4 [&_svg]:w-4 [&_svg]:shrink-0',
          isCollapsed &&
